...
fn limited_counter(limiter: speedbump.RateLimiter) -> Int {
  use <- speedbump.lazy_guard(limiter, 1000, fn(_) { 0 })
  1
}
...
pub fn basic_usage_test() {
  ...
  let assert Ok(limiter) =
    speedbump.start([
      speedbump.hits_per_second(hits: 10),
      speedbump.hits_per_minute(hits: 15),
    ])
...
pub fn a_rate_limit_with_an_invalid_configuration_immediately_fails_test() {
  let assert Error(start_error) =
    speedbump.start([speedbump.hits_per_second(hits: -10)])
...
pub fn each_limit_constructor_test() {
  ...
  let l = speedbump.hits_per_second(10)
  assert l.burst == 10
  assert l.tokens == 10
  ...
  let l = speedbump.hits_per_seconds(hits: 10, seconds: 2)
  assert l.burst == 10
  assert l.tokens == 10
  ...
  let l = speedbump.hits_per_minute(10)
  assert l.burst == 10
  assert l.tokens == 10
  ...
  let l = speedbump.hits_per_minutes(hits: 10, minutes: 5)
  assert l.burst == 10
  assert l.tokens == 10
  ...
  let l = speedbump.hits_per_hour(hits: 61)
  assert l.burst == 61
  assert l.tokens == 61
  ...
  let l = speedbump.hits_per_hours(hits: 61, hours: 2)
  assert l.burst == 61
  assert l.tokens == 61
  assert l.ns_per_token == 118_032_786_886
}

pub fn ask_speedbump_test() {
  let assert Ok(limiter) =
    speedbump.start([
      speedbump.hits_per_minute(60),
      speedbump.hits_per_hour(100),
    ])

  // No requests yet, should be able to make a request right away
  assert speedbump.ask(limiter, 1000, 1) == 0
  ...
  let wait = speedbump.ask(limiter, 1000, 1)
  assert wait > 0
  ...
  let wait = speedbump.ask(limiter, 1000, 2)
  assert wait > 0
